...
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `view_keeprecord` AS select `r`.`RecId` AS `RecId`,`t`.`TypeName` AS `Type`,`r`.`Count` AS `Count`,`u1`.`UnitsName` AS `Units`,`r`.`SubCount` AS `SubCount`,`u2`.`UnitsName` AS `SubUnits`,`r`.`RecordDatetime` AS `RecordDatetime`,`r`.`RecordDate` AS `RecordDate`,`r`.`UnitsId` AS `UnitsId`,`r`.`SubUnitsId` AS `SubUnitsId` from (((`tb_keeprecord` `r` left join `tb_keeptype` `t` on(((`r`.`TypeId` = `t`.`Id`) and (`t`.`Status` = 1)))) left join `tb_keepunits` `u1` on(((`r`.`UnitsId` = `u1`.`Id`) and (`u1`.`Status` = 1)))) left join `tb_keepunits` `u2` on(((`r`.`UnitsId` = `u2`.`Id`) and (`u2`.`Status` = 1)))) where (`r`.`Status` = 1);
